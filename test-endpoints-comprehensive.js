...
async function testAuthEndpoints() {
    ...
    console.log(`  Response: ${JSON.stringify(setupStatus.data).substring(0, 100)}`);
    ...
    if (loginRes.ok && loginRes.data.token) {
        ...
        console.log(`  Response: ${JSON.stringify(loginRes.data).substring(0, 150)}`);
    }
...
async function testPostsEndpoints() {
    ...
    if (authToken) {
        ...
        if (createPost.ok && createPost.data.post) {
            ...
            console.log(`  Response: ${JSON.stringify(createPost.data).substring(0, 100)}`);
        }
...
async function testCategoriesEndpoints() {
    ...
    if (authToken) {
        ...
        if (createCat.ok && createCat.data.category) {
            ...
            console.log(`  Response: ${JSON.stringify(createCat.data).substring(0, 100)}`);
        }
...
function printSummary() {
    ...
    console.log(`📊 Total: ${RESULTS.success.length + RESULTS.failed.length + RESULTS.skipped.length}`);

    const successRate = ((RESULTS.success.length / (RESULTS.success.length + RESULTS.failed.length)) * 100).toFixed(1);
